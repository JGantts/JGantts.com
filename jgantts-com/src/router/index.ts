import { applyRouteDocumentMeta } from './document-meta'
import {
  createRouter,
  createWebHistory,
...
router.afterEach((to, _from, failure) => {
  if (failure) return
  // The active post view owns article metadata, including revisioned canonicals.
  // Preserve server-rendered metadata until the view has its API data.
  if (to.name === 'Photos' && to.params.postId) return
