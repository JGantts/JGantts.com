...
test('retired Facebook jobs remain inert while Mastodon publication continues', async (t) => {
  const { database, posts, repository, service } = fixture();
  t.after(() => database.close());
  const postId = publishedPost(posts);
  const oldDate = '2000-01-01T00:00:00.000Z';
  for (const [index, state] of ['pending', 'processing', 'failed'].entries()) {
    const inserted = database.prepare(`
      INSERT INTO syndications (post_id, destination, remote_instance, state, publication_revision, idempotency_key, created_at, updated_at)
      VALUES (?, 'facebook', 'page:retired', 'pending', ?, ?, ?, ?)
    `).run(postId, index + 1, `retired-${state}`, oldDate, oldDate);
    const retiredId = Number(inserted.lastInsertRowid);
    database.prepare(`INSERT INTO publication_history (post_id, destination, publication_revision, state, created_at, updated_at)
      VALUES (?, 'facebook', ?, 'pending', ?, ?)`)
      .run(postId, index + 1, oldDate, oldDate);
    database.prepare(`
      INSERT INTO outbox_jobs (kind, aggregate_id, payload_json, state, available_at, locked_at, created_at, updated_at)
      VALUES ('facebook.publish_link', ?, ?, ?, ?, ?, ?, ?)
    `).run(String(retiredId), JSON.stringify({ syndicationId: retiredId }), state, oldDate, oldDate, oldDate, oldDate);
    assert.equal(repository.getById(retiredId), null);
    assert.throws(() => repository.retry(retiredId), /Only failed/);
  }
  const queued = service.queue(postId);
  const client = new FakeMastodonClient();
  const worker = new OutboxWorker(repository, client);
  assert.equal(await worker.runOnce(), true);
  assert.equal(await worker.runOnce(), false);
  assert.equal(client.publications.length, 1);
  assert.equal(repository.getById(queued.syndication.id)?.state, 'published');
  assert.deepEqual(repository.listForPost(postId).map((item) => item.destination), ['mastodon']);
  assert.ok(repository.listPublicationHistory(postId).every((item) => item.destination === 'mastodon'));
  assert.equal(database.prepare("SELECT SUM(attempt_count) FROM outbox_jobs WHERE kind = 'facebook.publish_link'").pluck().get(), 0);
  assert.equal(database.prepare("SELECT COUNT(*) FROM syndications WHERE destination = 'facebook'").pluck().get(), 3);
});
